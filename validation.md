...
Chrome, Internet Explorer and Edge. Note that the XML is not uploaded to the server, all the validation is implemented 
within the browser, so some details are browser-specific, notably the error messages for files that are not well formed
