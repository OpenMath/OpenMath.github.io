...
<xsl:template match="CDSignatures">
  ...
  <head>
    ...
    <link rel="stylesheet" href="../omcd.css" type="text/css"/>
  </head>
  <body>
    ...
    <p>
      ...
      <br/>
      CD Signature File:
      <a href="../../sts/{@cd}.sts"><xsl:value-of select="@cd"/>.sts</a>
      <br/>
      CD Signature as XML Encoded OpenMath: 
      <a href="../../omsts/{@cd}.omsts"><xsl:value-of select="@cd"/>.omsts</a>
    </p>
    <hr/>
    <xsl:apply-templates/>
  </body>
...
<xsl:template mode="display" match="OMOBJ">
  <xsl:apply-templates mode="display"/>
</xsl:template>



<xsl:template mode="display" match="OMA[OMS[@name='mapsto']]">
(
...
<xsl:template mode="display" match="OMA[OMS[@name='nassoc']]">
(
<xsl:apply-templates mode="display"  select="*[position()=last()]"/>
<sup>*assoc</sup>
)
...
<xsl:template mode="display" match="OMA[OMS[@name='nary']]">
(
<xsl:apply-templates mode="display"  select="*[position()=last()]"/>
<sup>*</sup>
)
...
<xsl:template mode="display" match="OMV">
  <xsl:text> </xsl:text>
...
<xsl:template mode="display" match="OMS[not(@cd='sts')]">
  <xsl:text> </xsl:text>
...
<xsl:template mode="display"
      match="OMS[@name='attribution']|
             OMS[@name='binder']|
             OMS[@name='Object']|
             OMS[@name='NumericalValue']">
  <xsl:text> </xsl:text>
